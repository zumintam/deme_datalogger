...
int modbus_send_raw_request_tid(modbus_t* ctx, const uint8_t* raw_req,
                                int raw_req_length, int tid) {
  sft_t sft;
...
int modbus_reply(modbus_t* ctx, const uint8_t* req, int req_length,
  ...
  switch (function) {
    ...
    case MODBUS_FC_WRITE_SINGLE_COIL: {
      ...
      /* Don't copy the CRC, if any, it will be computed later (even if
  ...
  /* Suppress any responses in RTU when the request was a broadcast, excepted
   * when quirk is enabled. */
